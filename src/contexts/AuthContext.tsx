...
// Define o tipo explicitamente para TypeScript
  ...
  try {
    ...
    const signIn = async (email: string, password: string) => {
      try {
        ...
        // Usar toast diretamente (função em vez do hook)
        toast({
    ...
    // Fallback provider quando o useState falha
    const fallbackValue: AuthContextType = {
      session: null,
      user: null,
      loading: false,
      signIn: async () => ({ error: new Error("Auth provider failed to initialize") }),
      signOut: async () => { console.log("Auth provider failed, sign out unavailable") }
    };

  ...
  useEffect(() => {
    if (historico && historico.length > 0) {
      ...
      const parsedData = historico.map(treino => {
        ...
        // Calcular a eficiência geral como número
  ...
  return (
    <Card>
            ...
            {selectedTreino && (
              <div>
                <div className="mb-4">
                  ...
                  <div className="mt-2">
                    <Badge variant={selectedTreino.presenca ? "default" : "destructive"}>
                      ...
                      {selectedTreino.avaliacoes.map((av, index) => (
                        <div key={index} className="bg-accent/40 p-3 rounded">
                          <div className="flex justify-between mb-1">
                            ...
                            <span>{Math.round(av.eficiencia)}% de eficiência</span>
                          </div>
                          <Progress 
                            value={av.eficiencia} 
                            className="h-2 mb-2"
                            indicatorColor={av.eficiencia > 70 ? "bg-green-500" : av.eficiencia > 40 ? "bg-yellow-500" : "bg-red-500"}
                          />
      ...
      <CardFooter className="flex justify-between border-t pt-4">
        ...
        {selectedTreino && (
          <p className="text-sm">
            Eficiência geral: <span className="font-medium">{Math.round(selectedTreino.eficienciaGeral)}%</span>
          </p>
        )}
      </CardFooter>
    </Card>

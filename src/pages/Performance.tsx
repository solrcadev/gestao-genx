...
const Performance = () => {
  try {
    ...
    const { data: performanceData, isLoading, error: queryError, refetch } = useQuery({
    ...
    // Se ocorreu um erro, mostrar o fallback
    if (hasError) {
      return <ErrorFallback error={error || new Error('Erro desconhecido na página de Desempenho')} />;
    }
    ...
    console.error("Erro ao renderizar componente Performance:", caughtError);
    return (
      <div className="error-container">
        <div className="error-icon">⚠️</div>
        <h2 className="error-title">Erro na página de Desempenho</h2>
        <p className="error-message">
          Ocorreu um erro ao renderizar a página de desempenho. Tente novamente mais tarde.
        </p>
        <button 
